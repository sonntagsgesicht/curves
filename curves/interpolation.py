...
from .curves import Curve as _Curve
from .numerics import solve as _solve
...
def fit(curve,
    ...
    if not isinstance(curve, _Curve):
        _curve = copy(curve)
        _val = 0.01
...
class base_interpolation(UserDict):
    ...
    def __copy__(self):
        cls = self.__class__
        return cls(self.x_list, self.y_list)

    def __str__(self):
        cls = self.__class__.__name__
        return f"{cls}({self.x_list}, {self.y_list})"

    def __repr__(self):
        cls = self.__class__.__name__
        return f"{cls}({self.x_list!r}, {self.y_list!r})"
...
class _default_value_interpolation(base_interpolation):
    ...
    def __str__(self):
        cls = self.__class__.__name__
        return (f"{cls}({self.x_list}, {self.y_list}, "
                f"default_value={self._default})")

    def __repr__(self):
        cls = self.__class__.__name__
        return (f"{cls}({self.x_list!r}, {self.y_list!r}, "
                f"default_value={self._default!r})")
...
class base_extrapolation:
    ...
    def __copy__(self):
        cls = self.__class__
        return cls(self.mid, left=self.left, right=self.right)

    def __str__(self):
        cls = self.__class__.__name__
        if self.left is None and self.right is None:
            return f"{cls}({self.mid}, left={self.left}, right={self.right})"
        if self.left is None:
            return f"{cls}({self.mid}, right={self.right})"
        if self.right is None:
            return f"{cls}({self.mid}, left={self.left})"
        return f"{cls}({self.mid}, left={self.left}, right={self.right})"

    def __repr__(self):
        cls = self.__class__.__name__
        if self.left is None and self.right is None:
            return (f"{cls}({self.mid!r}, "
                    f"left={self.left!r}, right={self.right!r})")
        if self.left is None:
            return f"{cls}({self.mid!r}, right={self.right!r})"
        if self.right is None:
            return f"{cls}({self.mid!r}, left={self.left!r})"
        return (f"{cls}({self.mid!r}, "
                f"left={self.left!r}, right={self.right!r})")


class _extrapolation(base_extrapolation):
    ...
    def __init__(self, x_list, y_list, mid=linear, left=None, right=None):
        ...
        super(_extrapolation, self).__init__(m_, l_, r_)


def extrapolation(x_list, y_list, mid=linear, left=None, right=None):
    mid = mid(x_list, y_list)
    if left:
        left = left(x_list, y_list)
    if right:
        right = right(x_list, y_list)
    return base_extrapolation(mid, left=left, right=right)
...
def waterfall(*mid, left=None, right=None):
    if 1 < len(mid):
        left = waterfall(*mid, left=left)
    return base_extrapolation(mid[0], left=left, right=right)
